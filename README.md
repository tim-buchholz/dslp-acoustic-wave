...
The serial drivers for Figures 4–7 are in `scripts/` and use the modules in
`src/`. The cluster drivers for Figures 8 and 9 are in `parallel/` and use the
...
The F8/F9 scripts are configured for the bwUniCluster module system and also
...
### Figure 4: Problem 125 with mass-lumped degree-1 FEM
...
`results/paper_DSTLP/f4_problem125_femml_deg1/`. Replace `<RUN_DIR>` below with
...
`results/tikz_DSTLP_F4_1D/problem125_FEMml_deg1_ell8_gamma1.tex`. Compile it with
...
### Figure 5: Problem 125 with standard FEM degrees 1–4
...
`results/tikz_DSTLP_F5_1D/problem125_FEM_deg*_ell8_gamma1.tex` files. Compile
them with
...
### Figure 6: Problem 125 overlap sweeps
...
The editable TikZ source is
`results/tikz_DSTLP_F6_1D/problem125_FEM_deg2_overlap_sweeps.tex`. Compile it
with
...
### Figure 7: Problem 223 in two dimensions
...
`results/tikz_DSTLP_F7_2D/problem223_FEM_deg2_DSTLP_ell4_ell8*.tex` files.
Compile them with
...
### Figure 8: Parallel pulse work-precision

Figure 8 is intended to run through Slurm on bwUniCluster. Detailed cluster
notes are in `parallel/F8_bwunicluster.md`. From the repository root, submit a
...
`parallel/results/F8_parallel_pulse_work_precision/<RUN_ID>/metrics_raw.csv`.
...
The TikZ sources are in `results/tikz_DSTLP_F8_parallel/`. Compile the two
...
Figure 9 is also intended for Slurm on bwUniCluster; see
`parallel/F9_bwunicluster.md` for retry and single-rank instructions. The raw F8
reference archive is not included. Keep `REF_ROOT` and `REF_BP` unset so every
normal F9 rank-count job generates a new CN reference in its own archive.
...
`parallel/results/F9_parallel_strong_scaling/<RUN_ID>/`. After transferring the
archive to the machine used for plotting, place it below
`results/paper_DSTLP/f9_parallel_strong_scaling/` and collect its per-rank data:
...
The TikZ sources are in `results/tikz_DSTLP_F9_parallel/`. Compile the scaling
plots with
