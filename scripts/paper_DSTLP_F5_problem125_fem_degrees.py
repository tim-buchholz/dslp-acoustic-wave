...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        default="results/paper_DSTLP/f5_problem125_fem_degrees",
    )
    parser.add_argument(
        "--serial",
        action="store_true",
...
def assert_distinct_degree_outputs(outputs: dict[str, dict[str, str]]) -> None:
    ...
    for degree, files in outputs.items():
        ...
        if digest in seen:
            raise RuntimeError(
                "F5 degree outputs are identical for "
...
def main() -> None:
    ...
    try:
        for degree in ARGS.degrees:
            print(f"=== Running F5 degree {degree} ===")
    ...
    manifest["elapsed_s"] = time.perf_counter() - started
    manifest_file.write_text(json.dumps(manifest, indent=2) + "\n")
    print(f"Archived F5 experiment in {run_dir}")
