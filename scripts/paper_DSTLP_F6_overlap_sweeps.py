...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        default="results/paper_DSTLP/f6_problem125_overlap_sweeps",
    )
    parser.add_argument(
        "--serial",
        action="store_true",
...
def main() -> None:
    ...
    try:
        ...
        for ell in ARGS.ell_sweep:
            print(f"=== F6 ell sweep: ell={ell}, gamma=1, min_pred=(2,1) ===")
        ...
        for gamma in ARGS.gamma_sweep:
            print(f"=== F6 gamma sweep: ell=4, gamma={gamma:g}, min_pred=(2,1) ===")
        ...
        for pair in MIN_PRED_SWEEP:
            print(f"=== F6 min-pred sweep: ell=4, gamma=1, min_pred={pair} ===")
    ...
    manifest_file.write_text(json.dumps(manifest, indent=2) + "\n")
    print(f"Archived F6 experiment in {run_dir}")
