...
## F1: Problem 125, FEM-ML Degree 1

This is the likely first figure of the paper. It compares Crank-Nicolson,
leapfrog, old domain splitting, and DSTLP for the mass-lumped 1D problem 125.
...
```text
results/paper_DSTLP/f1_problem125_femml_deg1/
```
...
```text
results/tikz_DSTLP_F1_1D/problem125_FEMml_deg1_ell8_gamma1.tex
```
...
## F2: Problem 125, Standard FEM Degrees 1-4

This figure family uses the same problem and time/domain parameters as F1, but
...
CSVs are identical because the first F2 driver version did not propagate
...
```text
results/paper_DSTLP/f2_problem125_fem_degrees/
```
...
```text
results/tikz_DSTLP_F2_1D/problem125_FEM_deg1_ell8_gamma1.tex
results/tikz_DSTLP_F2_1D/problem125_FEM_deg2_ell8_gamma1.tex
results/tikz_DSTLP_F2_1D/problem125_FEM_deg3_ell8_gamma1.tex
results/tikz_DSTLP_F2_1D/problem125_FEM_deg4_ell8_gamma1.tex
```

Each F2 plot includes:
...
## F3: Problem 125, DSTLP-CN Overlap Sweeps

This figure uses problem `125`, standard FEM degree `2`, the same `h=0.001`,
`T=5.0`, and `tau` range `1e-4` to `1e-1` as F1/F2. It plots only the
...
```text
results/paper_DSTLP/f3_problem125_overlap_sweeps/
```
...
```text
results/tikz_DSTLP_F3_1D/problem125_FEM_deg2_overlap_sweeps.tex
```

## F4: Problem 223, 2D DSTLP Comparison
...
```text
results/paper_DSTLP/f4_2d_dstlp_problem223/
```
...
```text
results/tikz_DSTLP_F4_2D/problem223_FEM_deg2_DSTLP_ell4_ell8.tex
results/tikz_DSTLP_F4_2D/problem223_FEM_deg2_DSTLP_ell4_ell8_a.tex
results/tikz_DSTLP_F4_2D/problem223_FEM_deg2_DSTLP_ell4_ell8_b.tex
```

<!--
## F5: Parallel MMS Work-Precision

This experiment is currently kept as a fallback and is probably not used in the
paper. It compares parallel CN, leapfrog, and DSTLP for a manufactured solution
on the unit square.

Detailed cluster instructions:

```text
parallel/F5_bwunicluster.md
```
...
| Parameter | Value |
|---|---|
| Data | MMS initial data and MMS RHS |
...
sbatch -p dev_cpu_il -N 1 -n 8 --time=00:30:00 \
  --output=../sbatch_F5_smoke_%j.log \
  --export=ALL,PROCS=8,NUM_TAUS=2,TAU_MAX=1e-2,TAU_MIN=1e-3,REPEATS=1 \
  ./f5_bwunicluster.sh
```

The first complete archive was:

```text
results/F5_parallel_work_precision/20260806T091939_F5_procs64_full
```

After pulling the archive back, the stable local data and TikZ files are:

```text
results/tikz_DSTLP_F5_parallel/data/f5_parallel_work_precision.csv
results/tikz_DSTLP_F5_parallel/f5_parallel_work_precision_time_loop.tex
results/tikz_DSTLP_F5_parallel/f5_parallel_work_precision_external_wall.tex
```

Compile:

```bash
cd results/tikz_DSTLP_F5_parallel
latexmk -pdf -outdir=out f5_parallel_work_precision_time_loop.tex
latexmk -pdf -outdir=out f5_parallel_work_precision_external_wall.tex
```
-->

## F6: Parallel Pulse Work-Precision
...
The historical raw F6 archive used for the paper is intentionally not included
in this reproduction bundle. A full run creates a new archive below
`results/F6_parallel_pulse_work_precision/`; the stable plot CSV used for the
...
results/tikz_DSTLP_F6_parallel/f6_parallel_pulse_work_precision_time_loop.tex
results/tikz_DSTLP_F6_parallel/f6_parallel_pulse_work_precision_external_wall.tex
results/tikz_DSTLP_F6_parallel/f6_parallel_pulse_timing_table.tex
```
...
## F7: Parallel Strong Scaling

This figure uses the F6 pulse configuration but fixes the timestep at
...
Raw F6 archives and their reference solutions are intentionally not included
in this reproduction bundle. Leave `REF_ROOT` and `REF_BP` unset when running
the normal F7 runner. It will generate a new CN reference inside each
rank-count archive. This makes the F7 sweep more expensive, but keeps it
self-contained.
...
```text
results/tikz_DSTLP_F7_parallel/data/f7_parallel_strong_scaling.csv
results/tikz_DSTLP_F7_parallel/data/f7_parallel_strong_scaling_iterative_gamg.csv
results/tikz_DSTLP_F7_parallel/f7_parallel_strong_scaling_time_loop.tex
results/tikz_DSTLP_F7_parallel/f7_parallel_strong_scaling_total_wall.tex
results/tikz_DSTLP_F7_parallel/f7_parallel_strong_scaling_time_loop_compare.tex
results/tikz_DSTLP_F7_parallel/f7_parallel_strong_scaling_total_wall_compare.tex
```
...
For the iterative CG-GAMG comparison, use a separate F7 archive and collect it
into:
