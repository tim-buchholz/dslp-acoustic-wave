# F7 bwUniCluster Parallel Strong-Scaling Experiment

This experiment uses the F6 pulse configuration and measures strong scaling for
...
The historical F6 archive is intentionally not included in this reproduction
bundle. Leave `REF_ROOT` and `REF_BP` unset for normal F7 jobs. Each rank-count
job then computes a new fallback CN reference inside its own archive. This is
more expensive than reusing an F6 reference, but makes the experiment
self-contained.

The dedicated one-rank workaround cannot generate a reference itself. Run at
least one normal F7 job first, then point its `REF_ROOT` to the current F7
...
```bash
RUN_DIR="results/F7_parallel_strong_scaling/$RUN_ID"
...
```bash
cd /pfs/data6/home/ka/ka_ianm/ka_wb9658/domainsplitting/parallel
RUN_ID=20260814T154227_F7_scaling_full
unset REF_ROOT REF_BP

for P in 80 100 128; do
  sbatch -p cpu_il -N 2 -n "$P" --time=12:00:00 \
    --output=../sbatch_F7_scaling_${P}_direct12h_%j.log \
...
for P in 80 100 128; do
  sbatch -p cpu_il -N 2 -n "$P" --time=04:00:00 \
    --output=../sbatch_F7_scaling_${P}_DSTLP_direct_%j.log \
...
```bash
cd /pfs/data6/home/ka/ka_ianm/ka_wb9658/domainsplitting/parallel
RUN_ID=$(date +%Y%m%dT%H%M%S)_F7_scaling_iter_gamg_full
echo "$RUN_ID"
unset REF_ROOT REF_BP
...
for P in 2 4 8 16 24 32 40 52 64 80 100 128; do
  ...
  sbatch -p cpu_il -N "$NODES" -n "$P" --time="$WALLTIME" \
    --output=../sbatch_F7_scaling_iter_gamg_${P}_%j.log \
...
```text
results/F7_parallel_strong_scaling/<RUN_ID>/
```
...
The normal F7 runner can hit a one-rank submeshing edge case. To obtain the
...
workaround locates that job's newly generated reference below the F7 archive.

```bash
cd /pfs/data6/home/ka/ka_ianm/ka_wb9658/domainsplitting/parallel
RUN_ID=<same F7 RUN_ID>
REF_ROOT="results/F7_parallel_strong_scaling/$RUN_ID"

sbatch -p cpu_il -N 1 -n 2 --time=04:00:00 \
  --output=../sbatch_F7_CN_n1_%j.log \
  --export=ALL,RUN_ID="$RUN_ID",RESUME=1,SUBMESH_PROCS=2,H=0.002,DEGREE=2,TAU=1e-3,REF_ROOT="$REF_ROOT",REPEATS=1 \
  ./f7_n1_cn_bwunicluster.sh
```
...
```bash
cd /pfs/data6/home/ka/ka_ianm/ka_wb9658/domainsplitting/parallel
RUN_ID=<iterative F7 RUN_ID>
REF_ROOT="results/F7_parallel_strong_scaling/$RUN_ID"

sbatch -p cpu_il -N 1 -n 2 --time=04:00:00 \
  --output=../sbatch_F7_CN_n1_iter_gamg_%j.log \
