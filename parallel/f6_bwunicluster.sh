#!/bin/bash
# F6 pulse work-precision experiment for bwUniCluster Ice Lake nodes.
...
RUN_ID=${RUN_ID:-$(date +%Y%m%dT%H%M%S)_F6_pulse_procs${PROCS}}
REPEATS=${REPEATS:-1}
T=${T:-1.0}
H=${H:-0.002}
DEGREE=${DEGREE:-2}
...
RESUME=${RESUME:-0}
EXPERIMENT_ROOT=${EXPERIMENT_ROOT:-results/F6_parallel_pulse_work_precision}
...
cp F6_bwunicluster.md "${EXPERIMENT_DIR}/F6_bwunicluster.md" 2>/dev/null || true
...
data = {
    ...
    "experiment": "F6_parallel_pulse_work_precision",
...
echo "=== F6 pulse run ${RUN_ID} on ${PROCS} MPI ranks ==="
...
echo "=== F6 finished: ${EXPERIMENT_DIR} ==="
