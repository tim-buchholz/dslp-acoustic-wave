...
# usage: mpirun -np N_PROCESSES [--oversubscribe] CrankNicolson
...
parser = argparse.ArgumentParser(
    prog=f"mpirun -np N_PROCESSES [--oversubscribe] CrankNicolson",
    description=">>> CN-FEM: Simulation Script of Crank-Nicolson applied to the wave equation with FEM space discretization (%(prog)s tau T) <<<",
    epilog=">>>  DS-CN Script  <<<",
)
...
linear_solver.add_argument(
    "--preconditioner", type=str, default="gamg", help="choose preconditioner"
)
linear_solver.add_argument(
...
output.add_argument("--solution_name", type=str, default="solCN")
# version
...
Omega = Domain(hdf5_source_global, comm)
...
boundary_dofs = dfx.fem.locate_dofs_topological(
    Omega.V, fdim, boundary_facets_global
)
...
###
#  Prepare Crank Nicolson solves
###
if subdomain_rank == 0:
    logger.info("preparing system solver and rhs for global Crank-Nicolson solves")
...
theta_ufl = dfx.fem.Constant(Omega.V.mesh, 1 / 4)
system_form = dfx.fem.form(
    mass_form(Omega.V, u)
    + theta_ufl * tau_ufl**2 * stiffness_form(Omega.V, u, kappa),
    jit_options=JIT_OPTIONS,
)

A_sys, solverSystem = LinearSolver(
    system_form, Omega.V, bcs=[bc]
)
...
rhs_form = dfx.fem.form(
    ...
    + tau_ufl**2
    / 4
    * (mass_form(Omega.V, fn) + mass_form(Omega.V, fn_plus))
    - tau_ufl**2 / 4 * stiffness_form(Omega.V, un, kappa),
    jit_options=JIT_OPTIONS,
)

b = dfx.fem.petsc.create_vector(Omega.V)  # rhs_form
comm.Barrier()
...
for n in range(N_t):
    ...
    # Crank Nicolson step on Omega.V
...
if args.rhs == "mms" and args.ic == "mms":
    ...
    error_L2_un, norm_L2_un_ref = error_norm(un, u_exakt(T), "L2", degree_raise=1, relative=True)
    error_H1_un, norm_H1_un_ref = error_norm(un, u_exakt(T), "H10", degree_raise=1, relative=True)
    error_L2_vn, norm_L2_vn_ref = error_norm(vn, v_exakt(T), "L2", degree_raise=1, relative=True)
...
if args.plot:
    ...
    if subdomain_rank == 0:
        ...
        filename = Path(f"plot/CrankNicolsonPlot.png")
