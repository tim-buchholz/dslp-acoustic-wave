#!/bin/bash
# F9 one-rank CN workaround: submesh with 2 MPI ranks, run CN with 1 rank.
...
RUN_ID=${RUN_ID:-$(date +%Y%m%dT%H%M%S)_F9_scaling}
REPEATS=${REPEATS:-1}
T=${T:-1.0}
TAU=${TAU:-1e-3}
H=${H:-0.002}
...
RESUME=${RESUME:-0}
EXPERIMENT_ROOT=${EXPERIMENT_ROOT:-results/F9_parallel_strong_scaling}
...
cp "${SCRIPT_ROOT}/F9_bwunicluster.md" "${EXPERIMENT_DIR}/F9_bwunicluster.md" 2>/dev/null || true
...
append_reference_errors() {
    ...
    "${MPI_RUN[@]}" python3 parallel/pulse_reference_errors.py \
        --candidate "${candidate}" \
...
data = {
    ...
    "experiment": "F9_parallel_strong_scaling_CN_n1_workaround",
    "procs": 1,
...
if [ -n "${REF_BP}" ]; then
    ...
    echo "Set REF_ROOT or REF_BP for the F8 reference solution; aborting."
    exit 1
fi
...
echo "=== F9 CN n=1 workaround, RUN_ID=${RUN_ID}, submesh ranks=${SUBMESH_PROCS}, CN ranks=1 ==="
...
echo "=== F9 CN n=1 workaround finished: ${EXPERIMENT_DIR} ==="
