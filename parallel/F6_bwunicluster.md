# F6 bwUniCluster Parallel Pulse Work-Precision Experiment

This is the pulse-data analogue of F5. It is separate from the MMS archive and
uses a numerically computed reference solution.
...
run is post-processed against it with `parallel/f6_reference_errors.py`.
...
Submit method chunks into the same `RUN_ID`, as for F5. Start with CN to build
the reference and global meshes:
...
```text
results/F6_parallel_pulse_work_precision/<RUN_ID>/
```
